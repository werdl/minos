...
pub fn init() {
    let res = unsafe { AcpiTables::search_for_rsdp_bios(minosAcpiHandler) };
    match res {
        Ok(acpi) => {
            ...
            if let Ok(dsdt) = &acpi.dsdt() {
                ...
                let handler = Box::new(minosAmlHandler);
...
#[derive(Clone)]
pub struct minosAcpiHandler;

impl AcpiHandler for minosAcpiHandler {
...
struct minosAmlHandler;

impl Handler for minosAmlHandler {

        ...
        match c {
            'h' => { // Enable
                for param in params.iter() {
                    match param[0] {
                        12 => sys::device::io::console::enable_echo(),
                        _ => return,
                    }
                }
            }
            'l' => { // Disable
                for param in params.iter() {
                    match param[0] {
                        12 => sys::device::io::console::disable_echo(),
                        _ => return,
                    }
                }
            }
...
pub fn init() {
    SERIAL.lock().init();
    sys::interrupts::idt::set_irq_handler(4, interrupt_handler);
}
...
fn interrupt_handler() {
    ...
    sys::device::io::console::key_handle(c);
}
